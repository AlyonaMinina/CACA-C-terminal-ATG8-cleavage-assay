...
// Create a shorter list containing .tif files only
tif_list = newArray(0);
for(s = 0; s < file_list.length; s++) {
    if(endsWith(file_list[s], ".tif")) {
        tif_list = Array.concat(tif_list, file_list[s]);
    }
}

// Inform the user about how many images will be analyzed from the selected folder
print(tif_list.length + " images were detected for analysis");
print("");

// Loop analysis through the list of .tif files
for (i = 0; i < tif_list.length; i++){
    path = original_dir + tif_list[i];
    ...
    print ("Processing image " + (i+1) + " out of " + tif_list.length + ":");
    print(title);
    print("");
    ...
    run("Close");
    Table.save(output_dir + "CACA ImageJ macro results" + ".csv");
    run("Close All");
}
